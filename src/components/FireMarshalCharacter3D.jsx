// ─────────────────────────────────────────────────────────────────────────────
// Fire Marshal "Sam" — 3D character.
//
// Ported verbatim from sarath200795/fire-marshal/src/components/Character3D.jsx.
// Flat-cartoon Sam: yellow hard hat, white shirt, coral/red hi-vis vest, dark
// trousers, glowing red safety baton in his right hand (rolled checklist in
// the left). Built from three.js primitives, procedurally animated by mode.
// No drei dep — only @react-three/fiber + three.
// ─────────────────────────────────────────────────────────────────────────────
...
const VEST = '#f73838', VEST_D = '#bd1414', STRIPE = '#fde68a';
...
function Rig({ mode = 'idle', facing = 1 }) {
    ...
    useFrame((state, delta) => {
        ...
        const set = (ref, axis, target) => {
            if (ref.current) ref.current.rotation[axis] += (target - ref.current.rotation[axis]) * k;
        };
        ...
        if (m === 'walk') {
            ...
            sLx = -0.9;  eLx = -1.3;
            sRx = -0.85; eRx = -1.2 + Math.sin(t * 8) * 0.14;
            hdx = 0.32;
        } else if (m === 'scratch') {
        ...
        if (glow.current && glow.current.material) {
            glow.current.material.emissiveIntensity = 0.55 + Math.sin(t * 6) * 0.4;
        }
    ...
    return (
        <group ref={root}>
            ...
            {/* torso = coral hi-vis vest over a white shirt */}
            ...
            {/* left arm holding the rolled checklist */}
            ...
            {/* right arm — pen in write mode, glowing red safety baton otherwise */}
            <group ref={sR} position={[0.52, 1.58, 0]}>
                ...
                <group ref={eR} position={[0, -0.44, 0]}>
                    ...
                    <group ref={baton} position={[0.04, -0.46, 0.1]} rotation={[0.18, 0, 0]}>
                        ...
                        <mesh ref={glow} position={[0, -0.34, 0]}>
                            <cylinderGeometry args={[0.055, 0.05, 0.6, 18]} />
                            <meshStandardMaterial color="#ff2a2a" emissive="#ff1414" emissiveIntensity={0.8} transparent opacity={0.82} />
                        </mesh>
            ...
            <group ref={head} position={[0, 1.62, 0]}>
                ...
                <mesh position={[0, 0.28, 0.45]} rotation={[Math.PI / 2, Math.PI, 0]}>
                    <torusGeometry args={[0.13, 0.028, 10, 20, Math.PI]} />
                    <meshStandardMaterial color={MOUTH} />
                </mesh>
                <mesh position={[0, 0.66, 0]}>
                    <sphereGeometry args={[0.55, 28, 18, 0, Math.PI * 2, 0, Math.PI / 2]} />
                    <meshStandardMaterial color={HAT} />
                </mesh>
                <mesh position={[0, 0.64, 0.08]} rotation={[Math.PI / 2, 0, 0]}>
                    <cylinderGeometry args={[0.66, 0.66, 0.1, 28]} />
                    <meshStandardMaterial color={HAT_D} />
                </mesh>
...
export default function FireMarshalCharacter3D({ mode = 'idle', size = 78, facing = 1 }) {
    const w = size;
