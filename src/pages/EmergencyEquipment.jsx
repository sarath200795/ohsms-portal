...
export default function EmergencyEquipment({ session, siteFilter, isGlobalUser, sites }) {
    const [equipment, setEquipment] = useState([]);
    const [loading, setLoading] = useState(true);
    const [view, setView] = useState('list'); // 'list' | 'form' | 'inspect'

    const [formData, setFormData] = useState({
        id: '', siteId: siteFilter === 'All' ? '' : siteFilter,
        type: 'Fire Extinguisher', location: '', serialNumber: '',
        lastInspection: new Date().toISOString().split('T')[0],
        nextInspection: '', status: 'Active', notes: ''
    });
    ...
    useEffect(() => {
        ...
        const fetchEq = async () => {
            const snap = await get(ref(rtdb, `organizations/${session.orgId}/emergencyEquipment`));
            if (snap.exists()) {
                setEquipment(Object.entries(snap.val()).map(([k, v]) => ({ firebaseKey: k, ...v })));
            } else {
                setEquipment([]);
            }
            setLoading(false);
        ...
        fetchEq();
    }, [session, view]); // Re-fetch when view changes back to list

    const visibleEquipment = useMemo(() => {
        return equipment.filter(e => {
            if (!isGlobalUser && session.assignedSite !== 'GLOBAL' && e.siteId !== session.assignedSite) return false;
    ...
    const getStatusBadge = (status, nextDate) => {
        ...
        return <span className="bg-emerald-900/30 text-emerald-400 border border-emerald-500/30 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-widest">Active & Compliant</span>;
    };

    if (loading) return <div className="p-8 text-center text-slate-400 animate-pulse"><i className="fas fa-spinner fa-spin mr-2"></i> Loading Equipment Registry...</div>;
            ...
            {view === 'list' && (
                <>
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-white flex items-center gap-3"><i className="fas fa-fire-extinguisher text-red-500"></i> Emergency Equipment</h2>
                            <p className="text-sm text-slate-400">Registry and inspection tracking for life-safety apparatus.</p>
                        </div>
                        {canEdit && <button onClick={() => { setFormData({ id: '', siteId: siteFilter === 'All' ? '' : siteFilter, type: 'Fire Extinguisher', location: '', serialNumber: '', lastInspection: new Date().toISOString().split('T')[0], nextInspection: '', status: 'Active', notes: '' }); setView('form'); }} className="bg-red-600 hover:bg-red-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg flex items-center gap-2 transition-transform active:scale-95"><i className="fas fa-plus"></i> Add Equipment</button>}
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="bg-slate-900/80 p-5 rounded-2xl border border-slate-700 shadow-lg flex items-center justify-between">
                            <div><p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Total Registered</p><h3 className="text-3xl font-black text-white">{stats.total}</h3></div>
                        ...
                        <div className="bg-slate-900/80 p-5 rounded-2xl border border-slate-700 shadow-lg flex items-center justify-between">
                            <div><p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Due &lt; 30 Days</p><h3 className="text-3xl font-black text-orange-400">{stats.expiringSoon}</h3></div>
                        ...
                        <div className="bg-slate-900/80 p-5 rounded-2xl border border-slate-700 shadow-lg flex items-center justify-between">
                            <div><p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Action Required</p><h3 className="text-3xl font-black text-red-500">{stats.actionNeeded}</h3></div>
                    ...
                    <div className="bg-slate-900/50 rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-950 border-b border-slate-800 text-[10px] uppercase tracking-widest font-bold text-slate-500">
                                <tr><th className="p-4 pl-6">Equipment</th><th className="p-4">Location</th><th className="p-4">Last Checked</th><th className="p-4">Status / Next Due</th><th className="p-4 pr-6 text-right">Actions</th></tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800/50 text-slate-300">
                                {visibleEquipment.map(e => (
                                    <tr key={e.firebaseKey} className="hover:bg-slate-800/40 transition-colors">
                                        <td className="p-4 pl-6">
                                            ...
                                            <div className="text-[10px] text-slate-500 mt-0.5">ID: {e.serialNumber || 'N/A'} | Site: {e.siteId}</div>
                                        </td>
                                ...
                                {visibleEquipment.length === 0 && <tr><td colSpan="5" className="p-10 text-center text-slate-500 italic">No equipment registered for this site.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </>
            ...
            {view === 'form' && (
                <div className="max-w-3xl mx-auto bg-slate-900/80 p-8 rounded-3xl border border-slate-700 shadow-2xl">
                    <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
                        <h3 className="text-xl font-bold text-white"><i className="fas fa-clipboard-list text-red-500 mr-2"></i> {formData.firebaseKey ? 'Edit Equipment' : 'Register New Equipment'}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500">
                                {TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Site</label>
                            <select value={formData.siteId} onChange={e => setFormData({ ...formData, siteId: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500">
                        ...
                        <div className="col-span-2">
                            ...
                            <input value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value })} placeholder="e.g. Ground Floor Kitchen, Next to Exit A" className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500" />
                        </div>
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Serial / Asset ID (Optional)</label>
                            <input value={formData.serialNumber} onChange={e => setFormData({ ...formData, serialNumber: e.target.value })} placeholder="e.g. FE-1042" className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500" />
                        </div>
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Current Status</label>
                            <select value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500">
                                {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Date of Last Inspection</label>
                            <input type="date" value={formData.lastInspection} onChange={e => setFormData({ ...formData, lastInspection: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500 font-mono" />
                        </div>
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Next Inspection Due Date</label>
                            <input type="date" value={formData.nextInspection} onChange={e => setFormData({ ...formData, nextInspection: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500 font-mono" />
                        </div>
                        <div className="col-span-2">
                            <label className="text-[10px] uppercase font-bold text-slate-400 block mb-2">Additional Notes</label>
                            <textarea rows="2" value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} placeholder="Any specific details..." className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-red-500 resize-none"></textarea>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-4 pt-4 border-t border-slate-800">
                        ...
                        <button onClick={handleSave} className="px-8 py-3 rounded-xl font-bold bg-red-600 text-white shadow-lg hover:bg-red-500 transition flex items-center gap-2"><i className="fas fa-save"></i> Save Record</button>
                    </div>
                </div>
            )}
