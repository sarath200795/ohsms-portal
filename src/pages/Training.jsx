...
export default function Training() {
            ...
            try {
                ...
                if (snap.exists()) {
                    ...
                    if (orgData.trainings) {
                        setTrainings(Object.entries(orgData.trainings).map(([k, v]) => ({ firebaseKey: k, ...v })));
                    }
                    ...
                    if (orgData.audits) {
                        Object.entries(orgData.audits).forEach(([key, item]) => {
                            if (item.findings) {
                                Object.entries(item.findings).forEach(([fIdx, find]) => {
                                    if (find && find.actions) {
                                        Object.entries(find.actions).forEach(([aIdx, act]) => {
                                            if (!act) return;
                                            const desc = act.action || act.act || '';
                                            if (checkDesc(desc)) {
                                                parsedCapas.push({
                                                    uid: `AUD-${key}-${fIdx}-${aIdx}`, source: 'Audit', sourceId: item.docId || key,
                                                    desc, owner: act.resp || act.owner || 'Unassigned',
                                                    due: act.target || act.due || 'N/A', status: act.status || 'Open',
                                                    siteId: item.siteId || 'Global',
                                                    dbPath: `organizations/${sess.orgId}/audits/${key}/findings/${fIdx}/actions/${aIdx}`
                                                });
                                            }
                                        });
                                    }
                    ...
                    if (orgData.consultations) {
                        Object.entries(orgData.consultations).forEach(([key, item]) => {
                            if (item.actions) {
                                Object.entries(item.actions).forEach(([idx, act]) => {
                                    if (!act) return;
                                    const desc = act.item || act.action || '';
                                    if (checkDesc(desc)) {
                                        parsedCapas.push({
                                            uid: `CONS-${key}-${idx}`, source: 'Consultation', sourceId: item.id || key,
                                            desc, owner: act.owner || 'Unassigned',
    ...
    const getMatrixCell = (userObj, topic) => {
        ...
        if (ROLE_REQUIREMENTS[userRole] && !ROLE_REQUIREMENTS[userRole].includes(topic)) {
            ...
            if (t.attendees) {
                t.attendees.forEach(a => {
                    if (a.userId === 'External' && !merged.find(u => u.name === a.name)) {
                        merged.push({ id: 'External', name: a.name, role: a.role || 'External / Contractor', assignedSite: t.siteId });
                    }
                });
            }
        });
    ...
    const filteredAlerts = useMemo(() => {
        ...
        return alerts.filter(a => {
            const u = users.find(x => x.name === a.userName);
    ...
    const initiateRetraining = (topic, usersToRetrain) => {
        ...
        const defaultAttendees = usersToRetrain.map(u => ({
            ...
            role: users.find(x => x.name === (u.userName || u.name))?.role || u.role || 'Employee',
            status: 'Attended'
        }));

        let derivedSite = getSmartSiteDefault();
        if (!derivedSite && usersToRetrain.length > 0 && usersToRetrain[0].assignedSite) {
            derivedSite = usersToRetrain[0].assignedSite;
        }
    ...
    const addAttendee = (type) => {
        if (type === 'external') {
            ...
            if (data.attendees.some(a => a.name.toLowerCase() === externalName.trim().toLowerCase())) return alert("Trainee is already in the list.");
            ...
            if (data.attendees.some(a => a.name === (userObj ? userObj.name : selectedUserToAdd))) return alert("Employee is already in the list.");
            const newAttendee = { userId: userObj ? userObj.id : 'Internal', name: userObj ? userObj.name : selectedUserToAdd, role: userObj ? userObj.role : 'Employee', status: 'Attended' };
            setData(prev => ({ ...prev, attendees: [...prev.attendees, newAttendee] }));
    ...
    const saveData = async () => {
        ...
        try {
            ...
            const snap = await get(dbRef);
            if (snap.exists()) {
                setTrainings(Object.entries(snap.val()).map(([k, v]) => ({ firebaseKey: k, ...v })));
            }
            setView('repo');
        } catch (e) {
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-950 text-white font-['Space_Grotesk'] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-8 print:hidden custom-scroll relative z-10">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
                                ...
                                <div className="flex-1 overflow-y-auto custom-scroll">
                                    <table className="w-full text-left text-sm text-slate-300">
                                        <thead className="bg-slate-950/80 backdrop-blur-md sticky top-0 text-[10px] uppercase font-bold text-slate-500 z-10 border-b border-slate-800">
                                            <tr><th className="p-4">Employee</th><th className="p-4">Topic</th><th className="p-4">Expiry</th><th className="p-4">Status</th><th className="p-4 text-right"></th></tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-800/50 bg-slate-950/40">
                                            ...
                                            {filteredAlerts.length === 0 && <tr><td colSpan="5" className="p-12 text-center text-emerald-400 italic font-bold">Excellent! No upcoming expirations.</td></tr>}
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            ...
                            <div className="glass-panel rounded-3xl overflow-hidden shadow-xl border border-slate-700 flex flex-col max-h-[500px]">
                                ...
                                <div className="flex-1 overflow-y-auto custom-scroll">
                                    <table className="w-full text-left text-sm text-slate-300">
                                        <thead className="bg-slate-950/80 backdrop-blur-md sticky top-0 text-[10px] uppercase font-bold text-slate-500 z-10 border-b border-slate-800">
                                            <tr><th className="p-4">Source</th><th className="p-4">Requirement / Topic</th><th className="p-4">Due Date</th><th className="p-4 text-right"></th></tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-800/50 bg-slate-950/40">
                                            {pendingTrainingCapas.map((capa, i) => (
                                                <tr key={i} className="hover:bg-slate-800/60 transition-colors">
                                                    <td className="p-4">
                                                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-widest border border-current shadow-sm ${capa.source === 'Incident' ? 'bg-orange-500/10 text-orange-400' : capa.source === 'Audit' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-blue-500/10 text-blue-400'}`}>
                                                            {capa.source}
                                                        </span>
                                                        {capa.source === 'Incident' && <span className="ml-2 text-[8px] bg-red-900/30 text-red-400 px-1.5 py-0.5 rounded uppercase font-bold border border-red-500/30">Global</span>}
                                            ...
                                            {pendingTrainingCapas.length === 0 && <tr><td colSpan="4" className="p-12 text-center text-slate-500 italic">No pending training CAPAs from other modules.</td></tr>}
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        ...
                        <div className="p-6 border-b border-slate-700 flex flex-wrap justify-between items-center gap-4 bg-slate-900/50 flex-shrink-0">
                            ...
                            <div className="flex gap-3 items-center flex-wrap">
                                ...
                                <div className="relative group">
                                    ...
                                    <input type="text" placeholder="Search Employee..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-slate-950 border border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white outline-none focus:border-blue-500 shadow-inner w-56 transition-colors" />
                                </div>
                        ...
                        <div className="flex-1 overflow-auto custom-scroll bg-slate-950/50">
                            <table className="w-full text-left text-sm text-slate-300 whitespace-nowrap min-w-max">
                                ...
                                <tbody className="divide-y divide-slate-800/80">
                                    ...
                                    {allMatrixRows.length === 0 && <tr><td colSpan={displayedTopics.length + 1} className="p-10 text-center italic text-slate-500 text-base">No data found matching your filters.</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
                                    ...
                                    {trainings.filter(t => filterSite === 'All' ? (isGlobalUser || allowedSiteCodes.has(t.siteId)) : t.siteId === filterSite).sort((a, b) => new Date(b.date) - new Date(a.date)).map(t => (
                                        <tr key={t.firebaseKey} className="hover:bg-slate-800/60 transition-colors">
                                            ...
                                            <td className="p-5 font-bold text-white text-base">
                                                {t.topic}
                                                {t.sourceCapaRef && t.sourceCapaRef.startsWith('Incident') && <span className="ml-2 text-[8px] bg-red-900/30 text-red-400 px-1.5 py-0.5 rounded uppercase font-bold border border-red-500/30">Global</span>}
                                            </td>
                                        ...
                                        <div className="space-y-4 mb-6">
                                            <div>
                                                ...
                                                <div className="flex gap-2">
                                                    <select value={selectedUserToAdd} onChange={e => setSelectedUserToAdd(e.target.value)} className="w-full text-sm font-bold text-white bg-slate-950 border border-slate-700 rounded-xl p-3 outline-none focus:border-emerald-500 shadow-inner transition-colors">
