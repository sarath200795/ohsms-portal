...
export default function Contractors() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (snap.exists()) {
                    ...
                    if (data.trainings) setGlobalTrainings(Object.values(data.trainings));
                    if (data.workPermits) setGlobalPermits(Object.values(data.workPermits));
                }
    ...
    const allWorkers = useMemo(() => {
        ...
        visibleContractors.forEach(c => {
            ...
            safeArr(c.workers).forEach(w => {
                // Find Trainings from Global Module
                ...
                const wInjuries = safeArr(c.incidents).filter(inc =>
                    inc.desc && inc.desc.toLowerCase().includes(w.name.toLowerCase())
                );
    ...
    const updateVendorDB = async (vendorKey, payload) => {
        try {
            ...
            if (activeVendor && activeVendor.firebaseKey === vendorKey) setActiveVendor(prev => ({ ...prev, ...payload }));
                                    ...
                                    <div className="bg-slate-950/50 p-6 rounded-2xl border border-slate-800 shadow-inner flex flex-col max-h-[550px]">
                                        ...
                                        {canEdit && (
                                            <div className="space-y-3 mb-6 bg-slate-900 p-4 rounded-xl border border-slate-700">
                                                ...
                                                <button onClick={addWorkerToForm} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg p-2 text-xs font-bold uppercase tracking-widest transition-colors shadow"><i className="fas fa-plus mr-1"></i> Add Employee</button>
                                            </div>
                                        )}
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scroll pr-2 space-y-2">
                                        {safeArr(activeVendor.workers).map(w => (
                                            <div key={w.id} className="p-3 rounded-xl border border-slate-700 bg-slate-900 shadow-sm relative group">
                                                <div className="text-sm font-bold text-white">{w.name} <span className="text-[9px] text-slate-400 font-normal ml-1">({w.role})</span></div>
