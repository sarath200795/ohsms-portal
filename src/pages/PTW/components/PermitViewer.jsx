...
import { PERMIT_TYPES, CHECKLIST_ITEMS, getTypeConfig } from '../../../utils/constants';
...
export default function PermitViewer({ permit, session, onCancel, onUpdate }) {
    ...
    const handleStatusChange = async (newStatus) => {
        ...
        try {
            ...
            onUpdate(); // Refresh the parent registry
        } catch (error) {
    ...
    const generatePDF = () => {
        try {
            ...
            doc.setFontSize(12);
            doc.setFont("helvetica", "bold");
            doc.text("Scope of Work:", 14, doc.lastAutoTable.finalY + 15);
            doc.setFontSize(10);
            ...
            const checklistBody = checklistKeys.length > 0
                ? checklistKeys.map(item => [item, permit.checklist[item] ? 'VERIFIED (YES)' : 'NO'])
            ...
            autoTable(doc, {
                ...
                body: checklistBody,
                theme: 'grid',
                headStyles: { fillColor: [tConfig.bg.replace('bg-', '').replace('/10', '') === 'red-500' ? 239 : 16, tConfig.bg.replace('bg-', '').replace('/10', '') === 'red-500' ? 68 : 185, tConfig.bg.replace('bg-', '').replace('/10', '') === 'red-500' ? 68 : 129] }, // Dynamic header color fallback
                columnStyles: { 1: { fontStyle: 'bold', halign: 'center', cellWidth: 40 } },
                willDrawCell: (data) => {
            ...
            if (finalY < 250) {
                ...
                doc.text("Issuer Signature", 30, finalY + 5);
        ...
        <div className="max-w-4xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-8 bg-slate-900/80 p-6 rounded-3xl border border-slate-700 shadow-xl backdrop-blur-md">
                <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-black shadow-lg ${tConfig.bg} ${tConfig.color} border ${tConfig.border}`}>
                ...
                <div className="flex gap-3">
                    <button onClick={generatePDF} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-blue-900/50 transition-transform active:scale-95 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="glass-panel p-6 rounded-3xl shadow-xl border border-slate-700 space-y-4">
                    <div>
                        <span className="text-[10px] uppercase font-bold text-slate-500 block">Site & Location</span>
                        <span className="text-white font-bold">{permit.siteId} - {permit.location}</span>
                    </div>
                    <div>
                        <span className="text-[10px] uppercase font-bold text-slate-500 block">Executing Agency</span>
                        <span className="text-white font-bold">{permit.contractorId === 'INTERNAL' ? 'Internal Team' : permit.contractorId || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="text-[10px] uppercase font-bold text-slate-500 block">Current Status</span>
                        <span className={`text-xs font-bold uppercase tracking-widest ${permit.status === 'Closed' ? 'text-emerald-400' : permit.status === 'Pending' ? 'text-orange-400' : 'text-blue-400'}`}>{permit.status}</span>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(permit.checklist || {}).map(([item, isChecked], idx) => (
                        <div key={idx} className={`p-4 rounded-xl border flex items-center gap-4 ${isChecked ? 'bg-emerald-900/10 border-emerald-500/30' : 'bg-red-900/10 border-red-500/30'}`}>
                            <i className={`fas ${isChecked ? 'fa-check-circle text-emerald-500' : 'fa-times-circle text-red-500'} text-xl`}></i>
                            <span className={isChecked ? 'text-slate-300' : 'text-slate-500 line-through'}>{item}</span>
                        </div>
                    ))}
                    {Object.keys(permit.checklist || {}).length === 0 && <p className="text-slate-500 italic">No checklist items recorded for this permit.</p>}
                </div>
            </div>
            ...
            <div className="flex justify-end gap-4 border-t border-slate-800 pt-6">
                {permit.status === 'Pending' && (
                    <button disabled={isProcessing} onClick={() => handleStatusChange('Approved')} className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest shadow-lg shadow-emerald-900/50 transition-transform active:scale-95 text-sm flex items-center gap-2">
                        {isProcessing ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-check-double"></i>} Approve Permit
                    </button>
                )}
                {permit.status === 'Approved' && (
                    <button disabled={isProcessing} onClick={() => handleStatusChange('Closed')} className="bg-slate-700 hover:bg-slate-600 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest shadow-lg transition-transform active:scale-95 text-sm flex items-center gap-2 border border-slate-500">
                        {isProcessing ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-lock"></i>} Close Permit
                    </button>
                )}
            </div>
        </div>
