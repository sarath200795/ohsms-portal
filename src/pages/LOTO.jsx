...
// ==========================================
// CONFIGURATION & CONSTANTS FROM HTML
// ==========================================
const ENERGY_CONFIG = {
...
export default function Loto() {
    ...
    const filteredProcedures = useMemo(() => {
        return procedures.filter(p => siteFilter === 'All' || p.facility === siteFilter);
    ...
    const filteredLogs = useMemo(() => {
        return logs.filter(log => {
            ...
            return proc && proc.facility === siteFilter;
        });
    }, [logs, procedures, siteFilter]);
    ...
    const startNewProcedure = () => {
        setProcForm({
            id: `LOTO-${Math.floor(1000 + Math.random() * 9000)}`,
            firebaseKey: '', facility: siteFilter !== 'All' ? siteFilter : (session.assignedSite || ''), location: '', description: '',
            date: new Date().toISOString().split('T')[0], status: 'Draft', author: session.user,
    ...
    const updateProcField = (field, val) => {
        setProcForm(prev => {
            ...
            if (field === 'facility' || field === 'description') {
                const f = next.facility ? next.facility.substring(0, 3).toUpperCase() : 'GEN';
                const d = next.description ? next.description.replace(/\s+/g, '').substring(0, 5).toUpperCase() : 'EQ';
                next.id = `${f}-${d}-${Math.floor(1000 + Math.random() * 9000)}`;
            }
            return next;
        });
    };
    ...
    const toggleChip = (idx, listName, item) => {
        setProcForm(prev => {
            const newSteps = [...prev.steps];
            const list = newSteps[idx][listName] || [];
            if (list.includes(item)) newSteps[idx][listName] = list.filter(i => i !== item);
            else newSteps[idx][listName] = [...list, item];
    ...
    const approveProcedure = async (proc) => {
        ...
        try {
            const payload = { ...proc, status: 'Approved', approvedBy: session.user, date: new Date().toISOString().split('T')[0] };
    ...
    // ==========================================
    // LOGIC: PDF EXPORT (JS-PDF)
    // ==========================================
    const generatePDF = (proc, tagsOnly = false) => {
        const doc = new jsPDF('p', 'mm', 'a4');

        const scanUrl = `${window.location.origin}${window.location.pathname}?execute=${proc.firebaseKey}`;
        const qrData = new QRious({ value: scanUrl, size: 250 }).toDataURL();
        ...
        doc.text(`SITE: ${proc.facility} | LOC: ${proc.location}`, 8, 34);
        doc.text(`EQUIP: ${proc.description} | ID: ${proc.id}`, 8, 40);
        ...
        doc.autoTable({
            ...
            body: proc.steps.map(s => [s.tag, `SOURCE: ${s.type}\nHW: ${(s.devices || []).join(', ')}\n\nMETHOD: ${s.isolate}`, "", s.verify]),
        ...
        doc.addPage();
        renderTagsPage(doc, proc, qrData);
        doc.save(`${proc.id}_PROCEDURE.pdf`);
    };
    ...
    const renderTagsPage = (doc, proc, qrData) => {
        ...
        proc.steps.forEach(s => {
            ...
            doc.text(`EQUIP: ${proc.description.substring(0, 25)}`, cx + 2, cy + 18);
    ...
    // --- EXECUTION VIEW (Mobile First Layout) ---
    if (currentView === 'execute') {
        ...
        if (!activeProc) {
            ...
            if (!activeProc) return <div className="p-10 text-center text-white bg-slate-950 h-screen font-['Space_Grotesk']">Procedure not found or access denied. <button onClick={() => navigate('/dashboard')} className="block mx-auto mt-4 bg-red-600 hover:bg-red-500 px-6 py-3 rounded-xl font-bold transition">Back to Hub</button></div>;
        }
        ...
        <div className="flex flex-col h-screen bg-slate-950 font-['Space_Grotesk'] text-white overflow-hidden relative">
            ...
            <header className="h-16 px-6 flex items-center justify-between border-b border-slate-800 bg-slate-900/80 backdrop-blur-md z-10 flex-shrink-0 no-print">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/ohs-tools')} className="text-slate-400 hover:text-white transition flex items-center gap-2"><i className="fas fa-arrow-left"></i> OHS Tools</button>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden md:flex items-center gap-2 bg-slate-900 border border-slate-700 rounded-xl px-3 py-1.5 shadow-inner">
                        ...
                        <select value={siteFilter} onChange={e => setSiteFilter(e.target.value)} className="bg-transparent border-none text-slate-200 font-bold text-xs outline-none shadow-none m-0 p-0 pr-2">
                ...
                {currentView === 'dashboard' && (
                    <div className="max-w-7xl mx-auto animate-fade-in space-y-8">
                        <div className="md:hidden mb-4">
                            ...
                            <select value={siteFilter} onChange={e => setSiteFilter(e.target.value)} className="w-full bg-slate-900 border border-slate-700 p-3 rounded-xl">
                                ...
                                <div key={i} className="glass-panel p-8 rounded-3xl flex flex-col xl:flex-row justify-between items-center gap-8 border border-slate-800 hover:border-slate-600 transition shadow-lg">
                                    <div className="flex-1 w-full">
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg ${proc.status === 'Approved' ? 'bg-emerald-900/30 text-emerald-400 border border-emerald-500/30' : 'bg-amber-900/30 text-amber-400 border border-amber-500/30'}`}>{proc.status}</span>
                                        </div>
                                    ...
                                    <div className="flex flex-wrap gap-3 w-full xl:w-auto">
                                        ...
                                        {proc.status === 'Draft' && (session.role === 'Manager' || session.role === 'Owner') && (
                ...
                {currentView === 'builder' && procForm && (
                    <div className="max-w-5xl mx-auto animate-fade-in pb-20">
                        <div className="flex justify-between items-center mb-8 bg-slate-900/80 p-6 rounded-3xl border border-slate-700 shadow-xl backdrop-blur-md">
                            <div>
                                <h2 className="text-3xl font-bold text-white mb-1"><i className="fas fa-drafting-compass text-emerald-400 mr-3"></i> {procForm.firebaseKey ? 'Edit LOTO Procedure' : 'Design LOTO Procedure'}</h2>
                        ...
                        <div className="glass-panel p-8 rounded-3xl shadow-2xl mb-10 border-t-4 border-blue-500">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <select value={procForm.facility} onChange={e => updateProcField('facility', e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3.5 rounded-xl text-white outline-none focus:border-blue-500 font-bold">
                                        <option value="">Select Site...</option>
                                        {sites.map(s => <option key={s.code} value={s.code}>{s.name}</option>)}
                                    </select>
                                </div>
                                <div><label className="text-[10px] uppercase font-bold text-slate-400 block mb-2 tracking-widest">Location / Area</label><input value={procForm.location} onChange={e => updateProcField('location', e.target.value)} placeholder="Boiler Room..." className="w-full bg-slate-950 border border-slate-800 p-3.5 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                                <div className="md:col-span-2"><label className="text-[10px] uppercase font-bold text-slate-400 block mb-2 tracking-widest">Equipment Description</label><input value={procForm.description} onChange={e => updateProcField('description', e.target.value)} placeholder="e.g. Conveyor Belt Motor A" className="w-full bg-slate-950 border border-slate-800 p-3.5 rounded-xl text-white outline-none focus:border-blue-500 text-xl font-bold" /></div>
                            </div>
                        ...
                        <div className="space-y-10">
                            {procForm.steps.map((step, idx) => (
                                <div key={step.id} className="glass-panel p-8 rounded-3xl relative border-l-8 border-slate-800 transition-colors shadow-xl" style={step.type ? { borderLeftColor: `rgb(${ENERGY_CONFIG[step.type].bg.join(',')})` } : {}}>
                                    {procForm.steps.length > 1 && <button onClick={() => removeStep(idx)} className="absolute -top-4 -right-4 bg-red-600 hover:bg-red-500 text-white w-10 h-10 rounded-full font-black shadow-lg border-[3px] border-slate-950 flex items-center justify-center transition-transform hover:scale-110"><i className="fas fa-times"></i></button>}
                                    ...
                                    <div className="flex justify-between items-center mb-8 border-b border-slate-700 pb-5">
                                        ...
                                        <select value={step.type} onChange={e => updateStep(idx, 'type', e.target.value)} className="w-auto text-sm font-bold uppercase bg-slate-900 border border-slate-700 p-3 rounded-xl text-white outline-none focus:border-blue-500">
                                    ...
                                    <div className="space-y-8">
                                        <div>
                                            ...
                                            <textarea value={step.isolate} onChange={e => updateStep(idx, 'isolate', e.target.value)} rows="2" placeholder="Turn valve 90 degrees..." className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white outline-none focus:border-blue-500 resize-none"></textarea>
                                        </div>

                                        <div>
                                            <label className="text-xs uppercase font-bold text-slate-300 block mb-3 tracking-widest">Verification Actions</label>
                                            <textarea value={step.verify} onChange={e => updateStep(idx, 'verify', e.target.value)} rows="1" placeholder="Press start button to confirm zero energy..." className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white outline-none focus:border-blue-500 resize-none"></textarea>
                                        </div>
                                        ...
                                        <div className="border-t border-slate-700 pt-6">
                                            ...
                                            {step.image ? (
                                                <div className="relative inline-block">
                                                    <img src={step.image} className="h-56 rounded-2xl border border-slate-600 object-cover shadow-lg" />
                                                    <button onClick={() => updateStep(idx, 'image', null)} className="absolute -top-3 -right-3 bg-slate-800 text-white rounded-full w-8 h-8 text-sm flex items-center justify-center border-2 border-slate-950 hover:bg-red-500 shadow-lg transition-colors"><i className="fas fa-times"></i></button>
                                                </div>
                                            ) : (
                                                <div className="bg-slate-900 border border-slate-700 p-2 rounded-xl inline-block shadow-inner">
                                                    <input type="file" accept="image/*" onChange={(e) => handleImageUpload(idx, e.target.files[0])} className="text-sm file:bg-slate-700 file:text-white file:border-none file:rounded-lg file:px-5 file:py-2.5 file:mr-4 file:font-bold file:cursor-pointer cursor-pointer w-auto bg-transparent border-none p-0 outline-none shadow-none text-slate-400" />
                                                </div>
                                            )}
                                        </div>
                                    </div>
